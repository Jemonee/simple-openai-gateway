...
const sliceColors = [
  ...
  'var(--hongfen-warning)',
  'var(--hongfen-danger)',
  'var(--hongfen-text-muted)',
  'var(--hongfen-primary-hover)',
  'var(--hongfen-border-strong)',
  'var(--hongfen-text)',
]
...
.channel-pie-slice { fill: currentColor; stroke: var(--hongfen-surface); stroke-width: 2; transform-box: fill-box; transform-origin: center; transition: opacity 140ms ease, transform 140ms ease; cursor: default; }
.channel-pie-slice.is-active { transform: scale(1.025); }
.channel-pie-slice.is-muted { opacity: .38; }
.channel-pie-slice:focus-visible { outline: none; stroke: var(--hongfen-text); stroke-width: 3; }
...
.channel-pie-legend li { display: grid; grid-template-columns: 10px minmax(0, 1fr) auto auto; align-items: center; gap: 10px; min-height: 40px; padding: 7px 8px; border-bottom: 1px solid var(--hongfen-border); font-variant-numeric: tabular-nums; }
.channel-pie-legend li:last-child { border-bottom: 0; }
.channel-pie-legend li.is-active { background: var(--hongfen-surface-muted); }
.channel-pie-legend li:focus-visible { outline: 2px solid var(--hongfen-primary); outline-offset: -2px; }
.channel-pie-legend i { width: 10px; height: 10px; }
.channel-pie-legend span { min-width: 0; overflow: hidden; color: var(--hongfen-text); font-size: 12px; text-overflow: ellipsis; white-space: nowrap; }
.channel-pie-legend strong { color: var(--hongfen-text); font: 650 12px var(--hongfen-font-mono); white-space: nowrap; }
.channel-pie-legend small { min-width: 48px; color: var(--hongfen-text-muted); font: 10px var(--hongfen-font-mono); text-align: right; }
.channel-pie-empty { display: grid; min-height: 338px; place-items: center; padding: 24px; color: var(--hongfen-text-muted); font-size: 12px; }
