        ...
        <el-table-column label="路由 / 调用身份" min-width="290">
          <template #default="scope">
            <div class="route-cell">
              ...
              <small><span>接口</span><code>{{ scope.row.latestEndpoint || '未知接口' }}</code><span>主模型</span><code>{{ scope.row.primaryModel || scope.row.latestModel || '未知模型' }}</code></small>
              <small v-if="(scope.row.latestModel && scope.row.latestModel !== scope.row.primaryModel) || scope.row.currentChannel"><span v-if="scope.row.latestModel && scope.row.latestModel !== scope.row.primaryModel">最近</span><code v-if="scope.row.latestModel && scope.row.latestModel !== scope.row.primaryModel">{{ scope.row.latestModel }}</code><span v-if="scope.row.currentChannel">上游</span><code v-if="scope.row.currentChannel">{{ scope.row.currentChannel.upstreamModel }}</code></small>
...
.active-rule { display: inline-flex; min-height: 34px; align-items: center; gap: 7px; padding: 0 10px; border: 1px solid var(--hongfen-border); border-radius: var(--hongfen-radius-control); color: var(--hongfen-text-muted); background: var(--hongfen-surface); font-size: 12px; }
.active-rule i { width: 7px; height: 7px; flex: none; border-radius: 50%; background: var(--hongfen-success); }
.active-query-bar { display: grid; grid-template-columns: minmax(240px, 520px) auto minmax(140px, 1fr); align-items: center; justify-content: start; gap: 8px; padding: 12px; border: 1px solid var(--hongfen-border); border-radius: var(--hongfen-radius-panel); background: var(--hongfen-surface); }
.active-query-bar .el-button { min-width: 88px; }
.active-count { display: inline-flex; align-items: center; justify-content: flex-end; gap: 6px; color: var(--hongfen-text-muted); font-size: 12px; font-variant-numeric: tabular-nums; }
.active-count svg { width: 15px; height: 15px; color: var(--hongfen-success); }
...
.active-table-panel .table-pagination { flex: none; min-height: 56px; align-items: center; background: var(--hongfen-surface); }
...
.session-origin { display: inline-flex; flex: none; align-items: center; gap: 5px; padding: 2px 6px; border: 1px solid var(--hongfen-border); color: var(--hongfen-text-muted); background: color-mix(in srgb, var(--hongfen-surface) 88%, transparent); font-size: 10px; line-height: 1.2; }
...
.route-cell > small { display: flex; min-width: 0; align-items: center; gap: 6px; overflow: hidden; color: var(--hongfen-text-muted); font-size: 10px; text-overflow: ellipsis; white-space: nowrap; }
...
.performance-cell .activity-line { display: flex; align-items: center; gap: 5px; color: var(--hongfen-success); }
